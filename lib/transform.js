...
export default function(inputType, outputType) {
  ...
  if (inputType === 'csv') {
    this.instreams = createInstreams(this.files, this.input, inputType);
    startTransformation = parseCSV;
  }
  if (outputType === 'json') {
    this.outstreams = createOutstreams(this.files, this.output, outputType);
    endTransformation = toJSON;
  }
